...
- `cargo test` in `src-tauri/` — Rust 单元测试(Rust 无独立测试框架,用 `#[cfg(test)]` 内嵌于各模块;49 条 + 11 ignored:capture 缩放/BGRA + DXGI 吞吐基准(ignored)、network 协议标签与 framing 往返 + 文件传输状态机/全双工/速率基准 + 信令长连接保活回归 + STUN 探测回环 + UDP 通道直连/中继回环 + UDP 半开看门狗回环 + 回退切换丢包基线重置 + **自连接双角色路由/自连接 TCP 会话回环**(local-host 免拦截的会话表按角色路由验证)、transport 分片重组/超时丢帧/重复分片/**关键帧门控** + 500 帧回环、hbb_client 配置与流参数(码率档位/旧配置迁移)+ 账号服务器解析、diagnostics 参数收敛/传输模式/哨兵/F2 评估 + TCP/UDP 真实链路自检(ignored)、input_injector code→VK 映射、virtual_display 注册表值、operation_log 读写、ffmpeg_hw 编码往返/基准/能力报告/画质档位码率差异(ignored))
...
- Rust commands (`src-tauri/src/`, snake_case, 注册于 `main.rs`):
  ...
  - `network.rs` — **真实远程控制协议(控制面 TCP + 视频面 UDP)**:TCP + 4 字节长度前缀 JSON 帧,消息以 `t` 字段区分(`hello/hello-ack/frame/mouse/key/clipboard/ping/pong/stream/udp-init/udp-init-ack/keyframe-request/udp-dead`),`PROTOCOL_VERSION = 4`。被控端 `run_host`/`serve_host`(单连接策略),控制端 `connect_peer`。TCP 模式视频帧为 base64 H.264/H.265 Annex-B(`Msg::Frame` 字段 `data`);UDP 模式经 `transport::split_packet` 分片走 `UdpChannel`(直连打洞失败回退中继 21119,再失败回退 TCP,会话不中断)。**UDP 丢帧恢复(F-1a)**:接收端重组器关键帧门控(丢帧后 delta 不透传),丢帧时经 TCP 控制面回发 `keyframe-request`,被控端下一帧强制编出 IDR(forced_idr,不等 GOP);**UDP 半开检测(F-1b)**:host 周期发 udp-keepalive 数据报,控制端看门狗(约 60 推帧周期无分片/保活)判定通道死亡 → 回退 TCP 拉流 + `udp-dead` 通知被控端 + `transport` 更新 "tcp" + op_log `udp_fallback`,会话不中断。控制端收到帧后**原样透传**前端 WebCodecs 解码(`RemoteFrameEvent { width, height, data, seq, dur, key, codec }`;UDP 模式 `dur = null`,前端显示 "--")。STUN:客户端 `stun_probe` 向信令 UDP 21115 发 Binding 取反射地址,host 注册 external 用反射地址。信令/中继对接:`connect_peer` 连接回退链 **直连(配置 LAN)→ 信令外部地址 → 中继兜底**(`open_transport`),host 启动时经 `signal_register_loop` 向信令注册+心跳;协议消息类型与 framing 复用 `server/` 的 `dcr_server` 库(路径依赖)。`SessionMetrics` 含 `transport`("tcp"/"udp"/"relay-udp")。- **本机自连接(local-host 允许)**:`connect_to_device` 不再拦截 `local-host`,直连 `127.0.0.1:{host_port}` 且被控端未运行时自动拉起 `start_host`;同进程内 Host/Client 双会话表并存(双表架构本为回环测试设计,自连接为其生产形态),控制端视角消息(Stream/UdpInit/DirList/FileRequest/剪贴板同步/控制端推文件)必须经 `session_send_client`/`session_send_side_pub(Client)` 显式走 Client 表,被控端视角回复与文件回传走 Host 表——`session_send` 在双表并存时优先路由 Host 表,错路由的消息 host 读循环永远收不到;`get_connection_state` 同理优先 Client 表(前端会话窗口按 peerId 匹配);自连接时 host 侧抑制 `connection-state` 事件(避免与 client 侧事件按 peerId 竞争)。设备列表本机条目状态固定 online(永远可连)。已用 CDP 驱动真实 app 全链路验证(连接/UDP 帧流/目录/断开)。
- **Windows 网络与抓屏三坑(真实复现修复,勿回潮)**:① `start_host` 的 std listener 转 tokio 前必须 `set_nonblocking(true)`——Windows 上 `from_std` 不校验阻塞模式,阻塞 socket 注册 IOCP 后 accept 完成事件可能永不被投递(host_accept 滞后数十秒直到其他 IO 活动触发,握手超时);② `spawn_blocking` 任务**无法被 `JoinHandle::abort` 中断**(tokio 语义),常驻循环(DXGI 抓帧)必须带 `CAPTURE_STOP` 原子停止标志自行退出,否则 stop/start 每次泄漏一个永不停止的循环;③ 抓帧循环已整体移到 `spawn_blocking` 专用线程(同步 D3D11 Map/编码每帧阻塞十几 ms,跑 async worker 会拖垮任务调度),循环内用 `std::thread::sleep`,`AcquireNextFrame` 超时空转分支同样要 sleep 防 100% CPU 忙等。
