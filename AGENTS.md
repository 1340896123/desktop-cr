...
- Rust commands (`src-tauri/src/`, snake_case, 注册于 `main.rs`):
  ...
  - `tray.rs` — 系统托盘(tauri `tray-icon` feature):常驻图标复用 `default_window_icon`,菜单 = 显示主窗口 / 允许他人协助(CheckMenuItem,与设置页同一接线:写 `host_enabled` + 真实启停 host)/ 文件传输(复用单例窗口)/ 退出;左键单击显示主窗口,右键弹菜单;关闭主窗口拦截 `CloseRequested` 隐藏到托盘(后台被控端继续运行),真正退出仅经托盘 `app.exit`;监听 `host-state` 事件同步勾选态与 tooltip。
