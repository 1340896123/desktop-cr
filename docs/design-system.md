...
### 远控窗口 RemoteSessionWindow
- 远控窗口为无边框窗口（Windows/Tauri），顶栏由页面自绘；高度 40px，白底 + 1px 浅描边，整条可拖拽，交互控件需标记为非拖拽区域
- 顶栏左侧：设备名胶囊（淡蓝底 + 主蓝图标 + 轻描边）+「超级屏」状态胶囊（黑底白字）
- 顶栏中部：显示屏选择下拉（灰底控件，hover 浅灰底），可选「添加显示屏」按钮
- 顶栏右侧：静音（图标按钮）/ 控制中心（文字按钮）/ 断开连接（红色主按钮）/ 窗口控制按钮组（最小化/最大化/关闭）
- 浮层（显示屏下拉、控制中心）：白底、圆角 12px、`--rd-shadow-popover`、`--rd-border-light`，hover/选中使用 `--rd-primary-soft`
- 远程画面区域：保持深色底以承载远程内容；性能浮窗使用半透明白底 + blur，避免遮挡画面且保证可读性

### 文件传输窗口 FileTransferWindow
- 文件传输窗口当前使用系统标题栏，内容区背景为 `--rd-bg`，整体留白 16–20px
- 顶部：页面标题（20–24px/700）+ 连接提示胶囊（未连接=灰底弱字；已连接=绿色文字 + 轻绿底）
- 双栏面板：白底卡片、圆角 12px、`--rd-shadow-card`、`--rd-border-light`；面板头部用 `--rd-bg` 形成层级
- 操作按钮：可用态使用 `--rd-primary`，hover 使用 `--rd-primary-hover`；不可用态用浅灰底 + 禁用光标
- 文件表格：表头 sticky，hover/选中使用 `--rd-primary-soft`；文字色遵循 `--rd-text-primary/secondary/muted`
- 传输队列：同卡片规范；进度条填充色使用 `--rd-primary`，失败态用 `--rd-destructive`，完成态用 `--rd-online`
